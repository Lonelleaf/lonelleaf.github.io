...
// listprogramload动画
// 倒序排列--i：排序后的序号
for (let j = document.getElementsByClassName("listprogram")
    .length; j > 0; j--) {
...
function onload() {
    for (
    let i = 0;
    i < document.getElementsByClassName("listprogram")
        .length;
    i++) {
...
function virgule(target, context, speed = 100) {
    ...
    function() {
        // 字符划分
        if (escape(contextArr[numVirgule])
            .indexOf("%u") < 0) {
